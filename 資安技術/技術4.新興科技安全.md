...
- 五大特色:
  - On-Demand Self-Service 自我隨需服務 => 供應商提供數種不同的服務，而由使用者自由選擇與使用。  
  - Broad Network Access 廣大網路存取 => 為滿足公用運算與隨選資源的能力，供應商要有一定以上的對Internet網路頻寬  
  - Resource Pooling 資源共享池 => 須具有一定規模的電腦硬體與網路資源以組合成資源池，以允許使用者自由的選擇運用。
  - Rapid Elasticity 快速彈性佈署 => 使用者在選取服務後，供應商應能在短時間內釋出指定的服務能量給使用者，並且可由使用者決定是否要擴充或縮減。  
  - Measured Service 計量服務 => 使用者在服務供應商取用的服務資源可以得到監測與評核，以作為使用者使用以及供應商計費的依據。  
