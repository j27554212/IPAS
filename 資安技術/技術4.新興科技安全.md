...
- 五大特色:
 ...
 四大部屬模式Deployment Models: 
  - 公用雲（Public cloud） =>是第三方提供一般公眾或大型產業集體使用的雲端基礎設施，系統服務提供者藉由租借方式提供客戶有能力部署及使用雲端服務。
  - 私有雲（Private cloud） =>是將雲基礎設施與軟硬體資源建立在防火牆內，以供機構或企業內各部門共享數據中心內的資源。
  - hybrid cloud  =>由兩個或更多雲端系統組成雲端基礎設施，這些雲端系統包含了私有雲、社群雲、公用雲等。
  - Community cloud =>
  -   
- 三大模型: 
...
- M1: Improper Platform Usage(平台使用不當)  
- M2: Insecure Data Storage(不安全的數據存儲)  
- M3: Insecure Communication(不安全的通信)  
- M4: Insecure Authentication(不安全的認證)  
- M5: Insufficient Cryptography(密碼學不足)  
- M6: Insecure Authorization(不安全的授權)  
- M7: Client Code Quality(客戶端代碼質量)  
-        =>當第三方可以傳遞不受信任的代碼作為應用程序將執行的輸入時，就會出現客戶端代碼質量問題。雖然這並不總是一個安全漏洞，但黑客也可以利用代碼質量問題來執行惡意代碼。  
- M8: Code Tampering ( 代碼篡改)  
-        =>代碼篡改意味著惡意行為者修改了源代碼、更改了應用程序包中的資源或重定向 API 調用以更改應用程序的行為。
-        =>攻擊者可能會使用代碼篡改將惡意軟件注入重新打包的應用程序並在移動應用程序商店中發布它們。
- M9: Reverse Engineering(逆向工程)  
- M10: Extraneous Functionality(額外的功能)  
...
## [OWASP IoT   10 - 2018](https://wiki.owasp.org/index.php/OWASP_Internet_of_Things_Project) 
- I1 [Weak Guessable, or Hardcoded Passwords](https://www.opencli.com/internet/router-common-login-password)
-           =>默認密碼較弱的物聯網設備容易受到網絡攻擊。物聯網設備製造商在啟動設備時必須注意密碼設置。  
-           =>設備不允許用戶更改默認密碼。  
- I2 Insecure Network Services 不安全的網路服務
-           =>在設備內運行的網絡服務可能對系統的安全性和完整性構成威脅。  
-           =>攻擊者可以利用網絡通信模型中存在的弱點成功地危害物聯網端點的安全。  
- I3 Insecure Ecosystem Interfaces
-           =>有多種接口，如 Web 界面、後端 API、雲和移動界面，可實現用戶與設備的流暢交互。  
-           =>缺乏適當的身份驗證、加密和數據過濾不當會對物聯網設備的安全性產生不利影響。   
- I4 Lack of Secure Update Mechanism
-           =>設備無法安全更新是列表中的第四個漏洞。
- I5 Use of Insecure or Outdated Components
-           =>使用不安全或過時的組件使用具有相關風險並威脅整個系統安全的第三方硬件或軟件。  
-           =>工業物聯網 (IIoT) 尤其受到難以更新和維護的系統的影響。  
- I6 Insufficient Privacy Protection
-           =>物聯網設備可能必須存儲和保留用戶的敏感信息才能正常運行。  
-           =>這些設備通常無法提供安全存儲，從而在被網絡犯罪分子入侵時導致關鍵數據洩露。  
- I7 Insecure Data Transfer and Storage [不安全的資料儲存](https://www.ithome.com.tw/news/145196)
-           =>在傳輸、處理或靜止期間處理敏感數據時缺乏加密是黑客竊取和暴露數據的機會。在涉及數據傳輸的任何地方都必須加密。   
- I8 Lack of Device Management
-           =>這是指無法有效保護網絡上的所有設備。它使系統面臨眾多威脅。無論涉及的設備數量或大小如何，都需要保護每一個設備免遭數據洩露。  
- I9 Insecure Default Settings
-           =>默認設置中的現有漏洞使系統面臨一系列安全問題。可能是固定密碼、無法跟上安全更新和過時組件的存在。   
- I10 Lack of Physical Hardening
-           =>缺乏物理加固很容易幫助懷有惡意的用戶遠程控制系統。    
-           =>缺乏物理加固，未能移除調試端口或移除存儲卡可能會使系統受到攻擊。  
